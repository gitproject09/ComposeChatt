package com.supan.presentation.utiles
...
import com.supan.domain.utils.states.UserState
...
var myAudioRecorder: MediaRecorder? =  null
 ...
 val myDir = File("$root/Compose Chat/Compose Recorders")
...
var voiceRecorderPermissions   =  arrayOf(Manifest.permission.MODIFY_AUDIO_SETTINGS,Manifest.permission.RECORD_AUDIO)
 ...
 fun sendVoiceMessage(
    ...
    myAudioRecorder!!.setOutputFile("$root/Compose Chat/Compose Recorders/${chatPageViewModel.recordFileName.value}")
    if (chatPageViewModel.isNewRecord.value) {
        ...
        chatPageViewModel.sendFile(
            ...
            fileType = "record",
            fileUri = Uri.fromFile(File("$root/Compose Chat/Compose Recorders/${chatPageViewModel.recordFileName.value}")),
        )
...
fun voiceMessage(
    ...
    if (list!!.contains(File("$root/Compose Chat/Compose Recorders/${messages.messageText}"))) {
...
fun downloadFile(fileUrl:String, fileName:String, applicationContext: ApplicationViewModel) {
    ...
    val myDir = File("$root/Compose Chat")
    if (!myDir.exists())
        myDir.mkdirs()
    val list = myDir.listFiles()
    if (list.contains(File("$root/Compose Chat/$fileName")))
    {

        //file already downloaded
        openFile(applicationContext.application,File("$root/Compose Chat/$fileName"))
